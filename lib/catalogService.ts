...
export async function generateProductCatalog(month: string) {
    ...
    doc.text('ONLINE BAR NAIROBI', 14, 25);

    doc.setFontSize(10);
    doc.setFont('helvetica', 'normal');
    doc.text(`PREMIUM DRINKS CATALOG - ${month.toUpperCase()} 2026`, 14, 34);
    ...
    const tableRows = products.map((p, i) => [
        ...
        formatPrice(p.price),
        p.stock > 0 ? 'AVAILABLE' : 'RESTOCKING'
    ]);

    autoTable(doc, {
        startY: 50,
        head: [['#', 'PRODUCT IDENTITY', 'CATEGORY', 'PRICE', 'STATUS']],
        body: tableRows,
        theme: 'striped',
        headStyles: {
        ...
        didDrawPage: () => {
            ...
            doc.text(
                '© 2026 ONLINE BAR™ - Nairobi Fast Dispatch Guaranteed. Visit onlinebar.co.ke for live stock.',
                14,
