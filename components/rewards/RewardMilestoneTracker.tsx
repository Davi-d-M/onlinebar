...
export default function RewardMilestoneTracker() {
    ...
    return (
        <Card className="p-10 rounded-[3.5rem] bg-slate-900 text-white space-y-10 relative overflow-hidden shadow-2xl border-none">
            {/* Background Glow */}
            <div className="absolute top-0 right-0 w-full h-full bg-gradient-to-br from-primary/10 via-transparent to-transparent pointer-events-none opacity-50" />
            ...
            <div className="relative z-10 space-y-10">
                <header className="flex justify-between items-start">
                    ...
                    <div className="h-14 w-14 rounded-2xl bg-white/10 flex items-center justify-center backdrop-blur-md border border-white/20 shadow-xl">
                ...
                <div className="space-y-6">
                    <div className="flex justify-between items-end px-2">
                        ...
                        <div className="text-right">
                            <span className="text-2xl font-black tracking-tighter">{tier.xp.toLocaleString()}</span>
                            <span className="text-xs font-black text-slate-500 uppercase tracking-widest ml-2">/ {tier.requiredXp.toLocaleString()} XP</span>
                        </div>
                    </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                    {tier.perks.slice(0, 4).map((perk, i) => (
                        <div key={i} className="flex items-center gap-3 p-4 bg-white/5 rounded-2xl border border-white/5 group hover:bg-white/10 transition-all cursor-default">
                            <ShieldCheck size={14} className="text-primary shrink-0" />
                            <span className="text-[10px] font-black uppercase tracking-tight text-slate-300 group-hover:text-white transition-colors">{perk}</span>
                        </div>
                    ))}
                </div>
            </div>
