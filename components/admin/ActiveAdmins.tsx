...
import { Activity, ExternalLink, GlassWater } from 'lucide-react';
            ...
            <div className="relative z-10 space-y-8">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        <div className="h-10 w-10 rounded-xl bg-primary/10 flex items-center justify-center text-primary shadow-sm"><GlassWater size={20} /></div>
                        <h3 className="text-[10px] font-black uppercase text-muted-foreground tracking-[0.4em]">Active Staff On-Shift</h3>
                    </div>
                    <div className="flex items-center gap-2 px-3 py-1 bg-emerald-50 text-emerald-600 rounded-full text-[9px] font-black uppercase border border-emerald-100">
                        <div className="h-1.5 w-1.5 rounded-full bg-emerald-500 animate-pulse" /> {activeAdmins.length} On Duty
                    </div>
                </div>

                <div className="space-y-4">
                    {activeAdmins.length === 0 ? (
                        <p className="py-6 text-center text-[10px] font-black text-slate-300 uppercase italic">Shift change in progress.</p>
                        ...
                        <div key={admin.session_id} className="p-4 rounded-2xl bg-slate-50 border border-slate-100 flex items-center justify-between group/session transition-all hover:bg-white hover:shadow-lg">
                            <div className="flex items-center gap-4">
                                <div className="h-10 w-10 rounded-xl bg-white border border-slate-200 flex items-center justify-center text-foreground font-black text-[10px] uppercase shadow-sm">
                                    {(admin.customer_name || 'ST').substring(0, 2)}
                                </div>
                                <div>
                                    <p className="text-xs font-black text-foreground uppercase">{admin.customer_name || 'Staff Member'}</p>
                                    <div className="flex items-center gap-2 mt-1">
                                        <p className="text-[8px] font-bold text-slate-400 uppercase tracking-widest">At: {admin.current_page.split('/').pop() || 'Bar Floor'}</p>
