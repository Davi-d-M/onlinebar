  ...
  return (
    <div className="p-8 space-y-10 bg-slate-50 min-h-screen text-left pb-40">
      <header className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-6 border-b border-slate-200 pb-8">
        <div>
          <h1 className="text-4xl font-black text-foreground uppercase tracking-tighter">Order fulfillment</h1>
          <p className="text-slate-500 text-sm font-medium mt-1">Manage beverage dispatch and generate customer receipts.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-left">
        ...
        <div className="bg-white p-8 rounded-[3rem] border border-slate-100 shadow-sm flex items-center gap-6 text-left group hover:shadow-xl transition-all">
          ...
          <div>
            <p className="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] leading-none mb-1">Delivered Bottles</p>
              ...
              <div className="flex-1 overflow-hidden">
                <div className="hidden lg:block overflow-x-auto no-scrollbar">
                    ...
                    <thead>
                        ...
                        <th className="px-8 py-5 min-w-[220px]">Beverage Details</th>
                        <th className="px-8 py-5 w-32">Authorized By</th>
                        {canSeeMoney && <th className="px-8 py-5 w-32">Order Margin</th>}
                        <th className="px-8 py-5 w-48">Dispatch State</th>
                            ...
                            <tr key={order.id} className={cn(
                                ...
                                <div className="flex flex-col text-left gap-3">
                                    {order.order_items && order.order_items.length > 0 ? (
                                        order.order_items.map((item) => {
                                            ...
                                            const isChilled = pName.toLowerCase().includes('wine') ||
                                                             pName.toLowerCase().includes('beer') ||
                                                             pName.toLowerCase().includes('juice') ||
                                                             pName.toLowerCase().includes('soda');
                                                ...
                                                <div key={item.id} className="pb-2 last:pb-0 border-b border-slate-50 last:border-none">
                                                    <div className="flex justify-between items-start gap-3">
                                                        <div className="flex flex-col gap-1">
                                                            <span className="font-black text-foreground uppercase text-[11px] block truncate max-w-[150px]">
                                                                {pName || `Item #${item.product_id}`}
                                                            </span>
                                                            <div className="flex items-center gap-1.5">
                                                                {isChilled ? (
                                                                    <span className="text-[7px] font-black uppercase text-blue-500 bg-blue-50 px-1.5 py-0.5 rounded flex items-center gap-1">
                                                                        <ThermometerSnowflake className="h-2 w-2" /> Chilled
                                                                    </span>
                                                                ) : (
                                                                    <span className="text-[7px] font-black uppercase text-amber-600 bg-amber-50 px-1.5 py-0.5 rounded flex items-center gap-1">
                                                                        <Coffee className="h-2 w-2" /> Dry Shelf
                                                                    </span>
                                                                )}
                                                            </div>
                                                        </div>
                                                    ...
                                                    <div className="flex items-center justify-between gap-2 mt-1.5">
                                                        <span className="text-[9px] text-slate-400 font-black uppercase tracking-widest">
                                                            x{item.quantity} Bottles {item.size && `(${item.size})`}
                                                        </span>
                                                    </div>
                                                </div>
                                            );
                                ...
                                <div className="flex flex-col gap-2.5">
                                    ...
                                    {order.status !== 'Delivered' && order.status !== 'Cancelled' && (
                                        ...
                                        >
                                            {order.rider_name ? `Runner: ${order.rider_name}` : 'Assign Runner'}
                                        </button>
                                    )}
                                </div>
                                </td>
      ...
      {assigningRiderId && (
          <div className="fixed inset-0 z-[110] flex items-center justify-center bg-slate-500/10 backdrop-blur-md p-4">
              <div className="bg-white rounded-[2.5rem] p-8 max-w-sm w-full shadow-2xl animate-in zoom-in-95">
                  <h3 className="text-xl font-black uppercase tracking-tighter text-foreground mb-6">Assign Bar Runner</h3>
                  <div className="space-y-4">
                      <div className="space-y-1">
                          <label className="text-[9px] font-black uppercase text-slate-400 ml-1">Runner Name</label>
                      ...
                      <div className="flex gap-2 pt-4">
                          <Button onClick={() => handleAssignRider(assigningRiderId!)} className="flex-1 rounded-xl h-12 bg-primary text-white font-black uppercase text-[10px] shadow-lg shadow-primary/20 hover:bg-primary/90 transition-all active:scale-95">Dispatch Runner</Button>
